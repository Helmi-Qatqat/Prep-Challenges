...
const objLat = (obj) => {
  let text = `my name is ${
    obj.firstName[0].toUpperCase() + obj.firstName.slice(1)
  } ${obj.lastName[0].toUpperCase() + obj.lastName.slice(1)} I am ${
    obj.age
  ...
  for (let i = 0; i < data.grades.length; i++) {
    for (let j = 0; j < data.grades[i].classes.length; j++) {
      data.grades[i].classes[j].avg = Math.floor(
        data.grades[i].classes[j].classScores.reduce((a, b) => a + b) /
          data.grades[i].classes[j].classScores.length
      );
    }
  }
  return data;
