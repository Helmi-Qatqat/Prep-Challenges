...
const roundDecimals = (arr) => {
    let output = []
    arr.forEach((e)=>output.push(Math.round(e)))
    return output
}
// -------------------------------------------------------------------------------------------------------
...
const employeesBonus = (arr) => {
    arr.forEach(e => {
    if(e.workHours > 8) {
        e.salary = `${+e.salary.slice(0 ,e.salary.length - 1) + 100}$`
    } else {
        e.salary = `${+e.salary.slice(0 ,e.salary.length - 1) + 50}$`
    }
    });
    return arr
}
...
const mostExpensive = (budget, mouseArray, keyBoardArray) => {
    ...
    mouseArray.forEach(e => {
        for(let i = 0; i < keyBoardArray.length; i++) {
            combination.push(e + keyBoardArray[i])
        }
    });
    return Math.max(...(combination.filter(e=> e <= budget)))
}
// -------------------------------------------------------------------------------------------------------
