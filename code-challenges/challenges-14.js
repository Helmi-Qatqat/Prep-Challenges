...
const replaceWords = (str) => {
    let repWord = ["I", "am", "was"]
    let toRepWord = ["We", "are", "were"]
    let testArray = str.split(" ")
    for(let i = 0; i < repWord.length; i++) {
        if(testArray.indexOf(repWord[i]) !== -1) {
            testArray.splice(testArray.indexOf(repWord[i]), 1, toRepWord[i])
        }
    }
